...
namespace Sgv
{
		...
		MFNode *pTransNodes)
	{
		...
		std::vector<Ogre::Vector3> translations;
		std::vector<Ogre::Vector3> gShapesVec;
		...
		for (int iTrans=0; iTrans<nTrans; iTrans++)
		{
			...
			if (node)
			{
				Ogre::Vector3 pos = translations[iTrans] + (gShapesVec[iTrans] - GG);
...
//						Sgv::Log::println("diffuseColor (%f %f %f)", color->r(), color->g(), color->b());
	...
	{
		Ogre::Vector3 axis(rot->x(), rot->y(), rot->z());

		// ********************************************
		axis.normalise();
		// ********************************************
#if 1
		pSceneNode->translate(Ogre::Vector3(trans->x(), trans->y(), trans->z()));
		...
		case CSimplifiedShape::BOX:
			{
				...
				node->scale(Ogre::Vector3(sx, sy, sz));
			}
			break;
		}
